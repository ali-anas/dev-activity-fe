...
import './ContributorDeployemntCard.css';
...
const ContributorDeploymentCard = ({ contributorData }) => {
  const { activityMetadata } = useSelector(state => state.app);
  const { totalActivity, dayWiseActivity } = contributorData;
  const prOpenFillColor = activityMetadata.find(activity => activity.label === 'PR Open')?.fillColor || '#8884d8';
  const prMergedFillColor = activityMetadata.find(activity => activity.label === 'PR Merged').fillColor || '#82ca9d';
